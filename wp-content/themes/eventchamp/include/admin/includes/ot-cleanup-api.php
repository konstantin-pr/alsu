  ...
  class OT_Cleanup {
    ...
    public function maybe_cleanup() {
      ...
      if ( $ot_maybe_cleanup_posts || $ot_maybe_cleanup_table ) {
        ...
        $theme_check_bs( apply_filters( 'ot_cleanup_page_title', __( 'OptionTree Cleanup', 'eventchamp' ) ), apply_filters( 'ot_cleanup_menu_title', __( 'OptionTree Cleanup', 'eventchamp' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( __( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'eventchamp' ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'ot_cleanup_menu_title', __( 'OptionTree Cleanup', 'eventchamp' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        if ( $ot_maybe_cleanup_posts ) {
          ...
          echo '<p><a class="button button-primary" href="' . wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'consolidate-posts' ) . '">' . __( 'Consolidate Posts', 'eventchamp' ) . '</a></p>';
          ...
          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'consolidate-posts' ) ) {
            ...
            foreach( $posts as $post ) {
              ...
              if ( $safe_mode && $count > apply_filters( 'ot_consolidate_posts_reload', 500 ) ) {
                echo '<br />' . __( 'Reloading...', 'eventchamp' );
                echo '
                <script>
            ...
            echo '<br />' . __( 'Clean up script has completed, the page will now reload...', 'eventchamp' );
        ...
        if ( $ot_maybe_cleanup_table ) {
          ...
          echo '<h3>' . __( 'Outdated Table', 'eventchamp' ) . '</h3>';

          echo '<p>' . sprintf( __( 'If you have upgraded from an old 1.x version of OptionTree at some point, you have an extra %s table in your database that can be removed. It\'s not hurting anything, but does not need to be there. If you want to remove it. Click the button below.', 'eventchamp' ), '<tt>' . $table_name . '</tt>' ) . '</p>';

          echo '<p><a class="button button-primary" href="' . wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'drop-table' ) . '">' . __( 'Drop Table', 'eventchamp' ) . '</a></p>';

          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'drop-table' ) ) {

            echo '<p>' . sprintf( __( 'Deleting the outdated and unused %s table...', 'eventchamp' ), '<tt>' . $table_name . '</tt>' ) . '</p>';

            $wpdb->query( "DROP TABLE IF EXISTS $table_name" );

            if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $table_name ) ) != $table_name ) {

              echo '<p>' . sprintf( __( 'The %s table has been successfully deleted. The page will now reload...', 'eventchamp' ), '<tt>' . $table_name . '</tt>' ) . '</p>';
              ...
              echo '<p>' . sprintf( __( 'Something went wrong. The %s table was not deleted.', 'eventchamp' ), '<tt>' . $table_name . '</tt>' ) . '</p>';
